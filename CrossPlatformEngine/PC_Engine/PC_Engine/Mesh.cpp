...
// Default Constructor
// --------------------------------------------------------
Mesh::Mesh() { myHelper->getInstance(); }
...
Mesh::Mesh(char* objFile, ID3D11Device *device, bool _hasNormalMap)
{
	...
	while (obj.good())
	{
		...
		else if (chars[0] == 'f')
		{
			...
			Vertex v1;
			v1.Position.x = positions[i[0] - 1].x;
			v1.Position.y = positions[i[0] - 1].y;
			v1.Position.z = positions[i[0] - 1].z;

			v1.UV.x = uvs[i[1] - 1].x;
			v1.UV.y = uvs[i[1] - 1].y;

			v1.Normal.x = normals[i[2] - 1].x;
			v1.Normal.y = normals[i[2] - 1].y;
			v1.Normal.z = normals[i[2] - 1].z;

			Vertex v2;
			v2.Position.x = positions[i[3] - 1].x;
			v2.Position.y = positions[i[3] - 1].y;
			v2.Position.z = positions[i[3] - 1].z;

			v2.UV.x = uvs[i[4] - 1].x;
			v2.UV.y = uvs[i[4] - 1].y;

			v2.Normal.x = normals[i[5] - 1].x;
			v2.Normal.y = normals[i[5] - 1].y;
			v2.Normal.z = normals[i[5] - 1].z;

			Vertex v3;
			v3.Position.x = positions[i[6] - 1].x;
			v3.Position.y = positions[i[6] - 1].y;
			v3.Position.z = positions[i[6] - 1].z;

			v3.UV.x = uvs[i[7] - 1].x;
			v3.UV.y = uvs[i[7] - 1].y;

			v3.Normal.x = normals[i[8] - 1].x;
			v3.Normal.y = normals[i[8] - 1].y;
			v3.Normal.z = normals[i[8] - 1].z;
			...
			if (facesRead == 12)
			{
				...
				Vertex v4;
				v4.Position.x = positions[i[9] - 1].x;
				v4.Position.y = positions[i[9] - 1].y;
				v4.Position.z = positions[i[9] - 1].z;

				v4.UV.x = uvs[i[10] - 1].x;
				v4.UV.y = uvs[i[10] - 1].y;

				v4.Normal.x = normals[i[11] - 1].x;
				v4.Normal.y = normals[i[11] - 1].y;
				v4.Normal.z = normals[i[11] - 1].z;
...
// Constructor
// --------------------------------------------------------
Mesh::Mesh(Vertex* verts, int numVerts, UINT *ind, int numInd, ID3D11Device *device)
{
	// Initialize fields
	vertBuffer = 0;
	indBuffer = 0;
	...
	CreateGeometry(verts, numVerts, ind, device);
}
...
// --------------------------------------------------------
void Mesh::CreateGeometry(Vertex* verts, int numVerts, UINT *ind, ID3D11Device *device)
{
...
void Mesh::CalculateTangents(Vertex* verts, int numVerts, unsigned int* indices, int numIndices)
{
	...
	for (int i = 0; i < numVerts; i++)
	{
		...
		XMVECTOR normal = XMLoadFloat3(&myHelper->F3ToXMF3(verts[i].Normal));
		XMVECTOR tangent = XMLoadFloat3(&myHelper->F3ToXMF3(verts[i].Tangent));
		...
		// Store the tangent
		verts[i].Tangent = myHelper->XMF3ToF3(temp);
	}
}
