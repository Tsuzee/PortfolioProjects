...
void Renderer::Draw(float deltaTime, float totalTime)
{
	...
	if (currentScene->opaque.size() > 0)
	{
		...
		{
			currentScene->opaque.at(i)->GetMat()->PrepareMaterial(myHelper->Mat4X4ToXMF4X4(currentScene->opaque.at(i)->GetWorldMat()), myHelper->Mat4X4ToXMF4X4(Cam->GetViewMatrix())
